...
The assistant sends the typed user scenario and a compact platform contract only. It does not read uploads, previews or source records. Its output is a proposal: platform-owned normalization, privacy checks, engine checks and semantic validation run before the UI accepts it. Explicit row counts are reapplied from the prompt, relationship keys become relationship-owned, unspecified child counts are derived from cardinalities, and executable date-order formulas are shown as reviewable reconciliations. All model-proposed provenance remains unconfirmed. One compact regeneration is allowed for errors the platform cannot safely reconcile. Generation still requires the user’s review action. A provider call can take up to 120 seconds; a correction can take another call.

The schema-only hosted smoke check is in `backend/benchmarks/check_hosted_assistant.py`. The demanding employee prompt check is in `backend/benchmarks/check_employee_assistant.py`. On 21 September 2026, the latter preserved 500 employees, covered all six supported base entities and requested fields, created five valid relationships, generated 130,795 rows with all constraints passing, and explicitly omitted unsupported monthly cross-table summaries instead of randomizing them. The saved result contains no credential or private record. These checks verify specific calls, not every future proposal.
