...
export function SpecEditor({spec,onApply,onDirty}:{spec:Spec;onApply:(s:Spec)=>Promise<void>;onDirty:(dirty:boolean)=>void}) {
  ...
  async function apply(){
    ...
    try {
      ...
      await onApply(result.spec??next);setDraft(result.spec??next);setDirty(false);onDirty(false);
  ...
  function addRelationship(){
    ...
    const tables=draft.tables.map((t,i)=>i===1?{...t,rows:null,columns:t.columns.some(c=>c.name===key)?t.columns:[...t.columns,{name:key,type:parent.columns.find(c=>c.name===pk)?.type??'integer',role:'foreign_key',provenance:proposed}]}:t);
    ...
    {advanced?<label className="stack">Specification JSON<textarea className="json-editor" rows={18} value={raw} onChange={e=>{setRaw(e.target.value);setDirty(true);onDirty(true);}} spellCheck={false}/></label>:<>
      ...
      {draft.tables.map((t,ti)=><fieldset className="editor-table" key={ti}><legend>Table {ti+1}: {t.name}</legend>
        ...
        {t.columns.map((c,ci)=>{
          ...
          return <details className="column-editor" key={ci} open={c.role!=='identifier'}><summary className="column-heading"><span className="eyebrow">Column {ci+1}</span><strong>{c.name||"Untitled column"}</strong><span className="small secondary">{c.type} · {c.role==='identifier'?'unique ID':c.role}</span></summary>
            <div className="grid grid-3 editor-fields">
              ...
              }}>{[['identifier','New unique identifier'],...(c.role==='foreign_key'?[['foreign_key','Set by a relationship']]:[]),['rule','My declared rule'],...(t.source.kind!=='none'?[['learned','Learn from uploaded data']]:[]),['constant','Always the same value'],['empty','Always empty'],['derived','Calculate using a formula'],...(c.role==='aggregate'?[['aggregate','Summarize related records']]:[])].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>
            </div>
            {c.role==='rule'&&<div className="grid grid-3 editor-fields">
              <label>Generation rule<select value={rule.kind??'integer_range'} onChange={e=>columnChange(ti,ci,{rule:{kind:e.target.value,start:0,end:100,values:['A','B'],provider:'word'}})}>{['integer_range','number_range','choice','faker','sequence','uuid4','date_range','timestamp_range'].map(v=><option key={v} value={v}>{({integer_range:'Whole number range',number_range:'Decimal number range',choice:'Choose from a list',faker:'Example text',sequence:'Number sequence',uuid4:'Unique ID (UUID)',date_range:'Date range',timestamp_range:'Date and time range'} as Record<string,string>)[v]}</option>)}</select></label>
